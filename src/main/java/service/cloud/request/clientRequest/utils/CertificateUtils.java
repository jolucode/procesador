...
//			throw e;
    ...
    public static synchronized byte[] getFile(String filePath) throws ConfigurationException, FileNotFoundException {
        if (logger.isDebugEnabled()) {
            logger.debug("+getCertificateInBytes() certificatePath: " + filePath);
        }
        byte[] certificateBytes = null;
        ByteArrayOutputStream baos = null;
        FileInputStream fis = null;
...
//			throw e;
            ...
            logger.error("getFile() Exception(" + e.getClass().getName() + ") ERROR: " + e.getMessage());
        }
        if (logger.isDebugEnabled()) {
            logger.debug("-getFile()");
        }
        return certificateBytes;
    } //getCertificateInBytes

    /**
     * Este metodo verifica que la contrasenia del certificado digital pueda abrirlo.
     *
     * @param certificate    El certificado digital en bytes.
     * @param certificatePwd La contrasenia del certificado digital.
     * @return Retorna un valor booleano que indica si el proceso fue satisfactorio o no.
     */
    public static boolean checkDigitalCertificate(byte[] certificate, String certificatePwd, String keystoreProvider, String keystoreType) {
        if (logger.isDebugEnabled()) {
            logger.debug("+checkDigitalCertificate()");
        }
        boolean flag = false;
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("checkDigitalCertificate() alias: " + alias);
            }
            ...
            logger.error("checkDigitalCertificate() ERROR: " + IVenturaError.ERROR_257.getMessage() + e.getMessage());
        }
        if (logger.isDebugEnabled()) {
            logger.debug("-checkDigitalCertificate()");
        }
