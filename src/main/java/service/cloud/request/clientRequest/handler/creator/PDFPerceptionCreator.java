    ...
    public byte[] createPerceptionPDF(PerceptionObject perceptionObj,
        ...
        if (null == perceptionObj) {
            ...
            try {
                 ...
                 */
                String outputPath = USER_TEMPORARY_PATH + File.separator
                        + docUUID + IPDFCreatorConfig.EE_PDF;
                JasperExportManager.exportReportToPdfFile(iJasperPrint,
                        outputPath);
                if (logger.isInfoEnabled()) {
                    logger.info("createInvoicePDF() [" + docUUID
                            + "] Se guardo el PDF en una ruta temportal: "
                            + outputPath);
                }
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("createInvoicePDF() [" + docUUID
                            + "] Se convirtio el PDF en bytes: " + pdfDocument);
                }
            } catch (Exception e) {
